...
def setOfWords2Vec(vocabList, inputSet):
    returnVec = [0]*len(vocabList) # initialize as 0
    for word in inputSet:
        if word in vocabList:
            returnVec[vocabList.index(word)] = 1 # set to 1 if exists
        else:
...
def bagOfWords2VecMN(vocabList, inputSet):
    ...
    for word in inputSet:
        if word in vocabList:
            returnVec[vocabList.index(word)] += 1 # deal with duplicated words
    return returnVec
