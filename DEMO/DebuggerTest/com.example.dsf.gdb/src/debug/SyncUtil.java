...
import debug.SyncUtil.DefaultTimeouts.ETimeout;
...
public class SyncUtil {
		...
		Runnable runnable = new Runnable() {
			...
			public void run() {
				DsfServicesTracker tracker = new DsfServicesTracker(
						Activator.getBundleContext(), fSession.getId());
			...
			fRunControl.getExecutor().submit(new Runnable() {
				...
				public void run() {
					...
					case STEP_RETURN:
						...
						break;
					default:
						Assert.isTrue(false, "Unsupported step type; " + stepType.toString());
					}
				}
			});
		} else {
	...
	public static String[] getBreakpointList(int timeout) throws Throwable {
		...
		fGdbControl.getExecutor().execute(query);
		MIBreakListInfo info = query.get(Activator.massageTimeout(timeout), TimeUnit.MILLISECONDS);
	...
	public static MIStoppedEvent resumeUntilStopped(int timeout) throws Throwable {
        IContainerDMContext containerDmc = SyncUtil.getContainerContext();
		return resumeUntilStopped(containerDmc, Activator.massageTimeout(timeout));
	}
	...
	public static boolean canResume(final IExecutionDMContext execDmc) throws Throwable {	
        ...
        boolean canResume = query.get(Activator.massageTimeout(500), TimeUnit.MILLISECONDS);
        return canResume;
	}
	...
	public static MIStoppedEvent waitForStop(int timeout) throws Throwable {
		...
		return eventWaitor.waitForEvent(Activator.massageTimeout(timeout));
	}
    ...
    public static IFrameDMContext getStackFrame(final IExecutionDMContext execCtx, final int level) throws Exception {
		...
		return query.get(Activator.massageTimeout(500), TimeUnit.MILLISECONDS);
    }
	...
	public static IExpressionDMContext getSubExpression(final IExpressionDMContext dmc)
		...
		assert 1 == subExpressions.length;
    ...
    static class DefaultTimeouts {
    	...
    	static int get(ETimeout timeout) {
    		...
    		assert value >= 0;
    		return Activator.massageTimeout(value);
    	}
    }
	...
	public static IContainerDMContext getContainerContext() throws InterruptedException, ExecutionException, TimeoutException {
				...
				fProcessesService.getProcessesBeingDebugged(
                    ...
                    protected void handleCompleted() {
                    	if (isSuccess()) {
                    		...
                    		Assert.isNotNull(contexts, "invalid return value from service");
                    		Assert.isTrue(contexts.length == 1, "unexpected number of processes");
                    		IDMContext context = contexts[0];    
                    		Assert.isNotNull(context, "unexpected process context type ");
		...
		fGdbControl.getExecutor().execute(query);
		return query.get(Activator.massageTimeout(2000), TimeUnit.MILLISECONDS);
	}
				...
				fProcessesService.getProcessesBeingDebugged(
                    ...
                    protected void handleCompleted() {
                    	if (isSuccess()) {
                    		...
                    		Assert.isNotNull(threads, "invalid return value from service");
	...
	public static IMIExecutionDMContext getExecutionContext(int threadIndex) throws InterruptedException, ExecutionException, TimeoutException {
		...
		Assert.isTrue(threadIndex < threads.length, "unexpected number of threads");
		Assert.isNotNull(threads[threadIndex], "unexpected thread context type ");
	...
	public static boolean canRestart() throws Exception {
        ...
        fGdbControl.getExecutor().execute(query);
        boolean canRestart = query.get(Activator.massageTimeout(500), TimeUnit.MILLISECONDS);
        return canRestart;
	}
	...
	public static MIStoppedEvent restart(final GdbLaunch launch) throws Exception {
		...
		if (!canRestart()) {
        	throw new CoreException(new Status(IStatus.ERROR, Activator.PLUGIN_ID, "Unable to restart"));
        }
        ...
        query2.get(Activator.massageTimeout(500), TimeUnit.MILLISECONDS);
    ...
    public static IVariableDMData[] getLocals(final IFrameDMContext frameDmc) throws Throwable {
    	...
    	fSession.getExecutor().execute(query);
    	IVariableDMData[] result = query.get(Activator.massageTimeout(500), TimeUnit.MILLISECONDS);
    	return result;
    }
